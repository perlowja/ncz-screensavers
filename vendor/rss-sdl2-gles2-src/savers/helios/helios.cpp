...
void doSaver(HWND hwnd){
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
	gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGB, TEXSIZE, TEXSIZE, GL_RGB, 
...
void handleCommandLine(int argc, char* argv[]){
	setDefaults();
	getArgumentsValue(argc, argv, std::string("-ions"), dIons, 1, 30000);
	getArgumentsValue(argc, argv, std::string("-size"), dSize, 1, 100);
	getArgumentsValue(argc, argv, std::string("-emitters"), dEmitters, 1, 10);
	getArgumentsValue(argc, argv, std::string("-attracters"), dAttracters, 1, 10);
	getArgumentsValue(argc, argv, std::string("-speed"), dSpeed, 1, 100);
	getArgumentsValue(argc, argv, std::string("-cameraspeed"), dCameraspeed, 0, 100);
	getArgumentsValue(argc, argv, std::string("-surface"), dSurface, 0, 1);
