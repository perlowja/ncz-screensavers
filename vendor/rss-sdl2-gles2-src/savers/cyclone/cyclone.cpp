...
void setProjection(){
	...
	static const int easterEgg = !rsRandi(500);
	if(easterEgg){  // Easter egg view
		glRotatef(90, 1, 0, 0);
...
void handleCommandLine(int argc, char* argv[]){
	setDefaults();
	getArgumentsValue(argc, argv, std::string("-cyclones"), dCyclones, 1, 10);
	...
	int b;
	if(getArgumentsValue(argc, argv, std::string("-stretch"), b, 0, 1) >= 0) dStretch = b;
	if(getArgumentsValue(argc, argv, std::string("-showcurves"), b, 0, 1) >= 0) dShowCurves = b;
}
#endif
