...
static void draw_scene_to_current_fbo(State *s, float t){
    ...
    /* Bind the PREVIOUS frame's texture into texture unit 0 */
    GLuint prev_tex = (s->fb_index == 0) ? s->tex_a : s->tex_b;
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, prev_tex);
    glUniform1i(s->u_prev, 0);

    /* u_fade is the *additional* per-frame fade on top of what the
     * fade shader already did (which is u_trail_persist). Together
     * they form the visible persistence curve. */
    glUniform1f(s->u_fade, 1.0f);  /* fade already done in the copy pass */
    ...
    /* Beat-like pulse: zero-crossing of sin at burst frequency, shaped
     * to be sharper than a pure sine so it feels musical. Also a
     * slower secondary beat for the texture cycling. */
    float pulse = 0.5f + 0.5f * sinf(t * s->v_burst_freq * 6.2831853f);
    float pulse2 = 0.5f + 0.5f * sinf(t * s->v_burst_freq * 6.2831853f * 0.5f + 1.7f);
    float pulseFinal = pulse * pulse;  /* sharpen */
    glUniform1f(s->u_pulse, pulseFinal);

    /* Flash: triggers briefly on leg boundaries (last 4% of a leg
     * plus first 2%) and decays fast. Throttled to once per ~6s. */
    float flash = 0.0f;
    float leg = s->v_journey_total / 5.0f;
    float x = fmodf(t, s->v_journey_total);
    int leg_idx = (int)(x / leg);
    float local = (x - (float)leg_idx * leg) / leg;
    /* Boundary detection: last 6% or first 6% of a leg */
    ...
    /* Throttle: never fire more often than every ~120 frames */
