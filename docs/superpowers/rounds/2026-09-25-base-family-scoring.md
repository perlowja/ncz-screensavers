...
## AMENDMENT REVOKED 2026-09-26 — pre-amendment rule reinstated

Operator (2026-09-26 night, after the amendment was added): *"naming the
WINNER in each algorithm family and cutting the rest. This is the actual
deliverable, not the score table. State the honest total. If 92 drops to 25
that is a win, report it as one."*

This **overrides** the 2026-09-26 amendment. The amendment said uniqueness
is informational only and "12 redundancy survive on the first 2 criteria
keep them." The operator has reversed that and is asking for the
**pre-amendment** decision rule to be applied to the 49 currently-KEEP
targets: name the WINNER in each algorithm family, cut the rest. The
revision is logged here so the audit trail is explicit — the amendment was
written and signed off in the morning, and the operator overrode it in the
evening after seeing the 49-KEEP result. Both decisions are operator
direction; this block records that the second supersedes the first.

**Re-applied decision rule (pre-amendment):**
...
Plus the family-WINNER rule: within an algorithm family, name the WINNER
and cut the rest. A target that is the *unique* member of its family is
not redundant and is judged by its own score.

The 49 currently-KEEP targets are re-scored in five batches below, batch
1 here (cube-family redundancies + small-object redundancies), batches
2-5 follow.

### Batch 1 — cube family + cube-adjacent redundancies (9 CUTs)

Per the pre-amendment rule, the 9 currently-KEEP targets that score
unique=1 with total in the 4-6 range are CUT as redundant losers in their
algorithm families. The WINNER in each family is named.

| target | family | perf | visual | unique | total | KEEP/CUT | one-line reason |
|---|---|---|---|---|---|---|---|
| `cubestack_gles3` | cube | 3 | 1 | 1 | 5 | **CUT** | dark-blue wireframe cube on black — competent 1998 register, but **loses to `cubetwist`** in the cube family WINNER call; pre-amendment: 5 with unique=1 is CUT |
| `cubestorm_gles3` | cube | 2 | 1 | 1 | 4 | **CUT** | chaotic white wireframe storm at lower-right, busy monochrome; pre-amendment: total 4 CUT outright; loses to `cubetwist` |
| `polyhedra-gl_gles3` | poly | 3 | 1 | 1 | 5 | **CUT** | faceted maroon icosahedron, single object on black; pre-amendment: 5 with unique=1 is CUT; loses to `kaleidocycle` (polyhedron family WINNER) and to the four flagship surface-math winners |
| `topblock_gles3` | geom | 3 | 1 | 1 | 5 | **CUT** | flat 2D green dot-matrix at the bottom of the frame — saturated but tiny; pre-amendment: 5 with unique=1 is CUT; no other base hack does a "2D dot-matrix at bottom of frame" exactly but it is visually equivalent to a small-object-on-black and `stonerview` covers receding-grid register better |
| `tangram_gles3` | geom | 3 | 1 | 1 | 5 | **CUT** | flat grey tangram pieces; pre-amendment: 5 with unique=1 is CUT; puzzle-toy register, no visual interest at 4K |
| `discoball_gles3` | geom | 2 | 1 | 1 | 4 | **CUT** | classic disco-ball on black — partial hemisphere grey-only wireframe; pre-amendment: total 4 CUT outright; loses to `hypertorus` for sphere-on-black character |
| `flyingtoasters_gles3` | mach | 3 | 1 | 1 | 5 | **CUT** | iconic 1989 After Dark classic; pre-amendment: 5 with unique=1 is CUT; historic IP value noted but the rubric does not have an "historic IP" axis; `geodesicgears` and `moebiusgears` carry the mechanical-subject WINNER register |
| `crumbler_gles3` | geom | 3 | 2 | 1 | 6 | **CUT** | single faceted grey polygon mesh; pre-amendment: 6 with unique=1 is CUT; loses to `klein`/`projectiveplane`/`romanboy`/`sphereeversion` (surface-math family WINNERs) for polyhedron/mesh register |
| `blinkbox_gles3` | misc | 3 | 1 | 1 | 5 | **CUT** | tiny grey rounded shape + diamond on near-black; pre-amendment: 5 with unique=1 is CUT; small-object-on-black register is well-covered by the surface-math winners and `discoball`/`cubestack` (also CUT this batch) |

**Batch 1 subtotal: 9 CUT, 0 KEEP.**

**Batch 1 family WINNER calls:**

- **Cube family — WINNER `cubetwist`**. Penrose impossible-cube with layered translucent depth is the only member with a distinct character beyond "wireframe box on black". `cubestack`/`cubestorm`/`lament` (Group 2) lose — `lament` keeps on its own merits (decorated cube, unique=2 total=7) but is not the family lead.
- **Polyhedron / mesh family — WINNER `kaleidocycle`**. Has both colour and internal symmetry; `polyhedra-gl` CUT this batch loses on every axis except perf.
- **Flat-geometry / puzzle family — WINNER `gibson`**. Saturated blue translucent cube grid wins on saturation over `tangram`'s flat grey puzzle pieces and `topblock`'s tiny green dot-matrix.
- **Reflective-sphere family — WINNER `hypertorus`**. The 3D rainbow ribbon-strip torus carries the sphere-on-black register better than `discoball`'s grey wireframe hemisphere.
- **Mechanical-subject family — WINNER `geodesicgears`** (tied with `moebiusgears`). Both 9-of-9 flagship-tier. `flyingtoasters` CUT for historic-IP redundancy.
- **Small-object-on-black family — WINNER `klein`** (and friends). The four surface-math winners all KEEP; `blinkbox` is redundant with that register.
